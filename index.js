...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Takamura\n' // Seu nome
            + 'ORG:Takamura;\n' // Nome do Bot
            + 'TEL;type=CELL;type=VOICE;waid=558695023910:+55 86 95023910\n' // Numero do Whatsapp
            + 'END:VCARD'
prefix = '*','#','+'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Escaneie Codigo QR'))
	})
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: '[̲̅ρ̲̅є̲̅я̲̅α̲̅ ̲̅α̲̅є̲̅ ̲̅ρ̲̅σ̲̅я̲̅я̲̅α̲̅ ̲̅,̲̅ ̲̅т̲̅σ̲̅ ̲̅ρ̲̅я̲̅σ̲̅c̲̅є̲̅s̲̅s̲̅α̲̅я̲̅ ̲̅σ̲̅ ̲̅C̲̅σ̲̅м̲̅α̲̅и̲̅d̲̅σ̲̅]',
				...
				only: {
					...
					premium: '[❗] ESTE PEDIDO É SO PARA *USUÁRIOS VIPS*',
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD †ลкลмµяล BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] †ลкลмµяล? Este é um recurso especial para o Takamura ❌',
					ownerB: '[❗] †ลкลмµяล? Este é um recurso especial para o Takamura ❌',
					userB: `──「 LISTA 」──\nOlá Consagrado !\nDesculpe, irmão. Você não está registrado como amigo do Takamura. Registre-se para fazer amizade com o bot †ลкลмµяล por meio, \n\nCommand : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar †ลкลмµяล|17\n\n──「 BOT MAN 」──`,
			...
			const ownerNumber = ["558695023910@s.whatsapp.net","558695023910@s.whatsapp.net"] // Recoloque o seu numero
			const mod = [ownerNumber,"558695023910@s.whatsapp.net","558695023910@s.whatsapp.net"]// não sei o que eu isso 
			const adminbotnumber = ["558695023910@s.whatsapp.net","558695023910@s.whatsapp.net"]// admin bot numero
			const frendsowner = ["7777"]// amigo do criador 
			const premium = ["558695023910@s.whatsapp.net","558695023910@s.whatsapp.net"]
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("*meliodas")){
			...
			if (messagesC.includes("@558695023910")){
			client.updatePresence(from, Presence.composing)
	...
	if (messagesC.includes("botvagbaundo")){
			client.updatePresence(from, Presence.composing)
			...
			switch(command) {
				...
				case 'menu1':
				    ...
				    try {    
						...
						reply('Erro Tente Novamento Meu Chapa(a)')
					}
					break
					case 'lista':
				...
				case 'walpaperhd':
                    ...
                    if (!isGroup) return reply(from, 'Este Comando E Apenas Usado Em Um Grupo', id)
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'modoanime':
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`✓Ativado com sucesso o modo anime no grupo, Yamete Curasai\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						anime.splice(from, 1)
						fs.writeFileSync('./database/json/anime.json', JSON.stringify(anime))
						reply(`\`\`\`✓Modo anime desativado com sucesso no grupo, Oh Noooo\`\`\` *${groupMetadata.subject}*`)
					} else {
				...
				case 'modonsfw':
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`✓Ativado com sucesso o modo nsfw no grupo 18+\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
						reply(`\`\`\`✓Modo nsfw desativado com sucesso no grupo , Queria Uma Bronha De Leve\`\`\` *${groupMetadata.subject}*`)
					} else {
						reply('On para ativar, Off para desligar')
					}
					break
...
//@darkYT					
    ...
    break
				...
				case 'premiumlist':
					...
					break
                  case 'qrcode':
        			if (!isPremium) return reply('Você não é um Membro VIP, entre em contato com o proprietário ou digite *.Daftarvip* para adquirir o acesso VIP!' ,text, { quoted: mek })
			...
			case 'instaimg':
				...
				break
                                case 'afk':
                                        ...
                                        break 
				case 'indo10':
					if (!isPremium) return reply('Você não é um Membro VIP, entre em contato com o proprietário ou digite *.daftarvip* para comprar acesso VIP!' ,text, { quoted: mek })
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS †ลкลмµяล 𝜝𝛩͢𝑇 ˃͜˂* 」\n\`\`\``
					no = 0
...
// recursos anime, random @✞︎𝙼𝚎𝚕𝚒𝚘𝚍𝚊𝚜✞︎ domina vida
          ...
          reply('Espere..')
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro VIP, entre em contato com o Takamura ou digite *.Daftarvip* para adquirir o acesso VIP!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT †ลкลмµяล 𝜝𝛩͢𝑇 ✨* 」\n*│+ wa.me/558695023910*╰──────*「 *†ลкลмµяล 𝜝𝛩͢𝑇* 」*────*\n\n*_SE QUER SER ADMIN DO BOT †ลкลмµяล 𝜝𝛩͢𝑇_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *VIP USER👑* 」\n*│+ wa.me/558695023910/*╰──────*「 * †ลкลмµяล 𝜝𝛩͢𝑇* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO †ลкลмµяล 𝜝𝛩͢𝑇 BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'boanoite':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.makeagif.com/media/3-24-2019/9tgED-.gif`)
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*Canal Do Takamura:*\n\n em breve'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
